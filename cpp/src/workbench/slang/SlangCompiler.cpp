...
namespace {
    ...
    QString blobText(slang::IBlob* blob)
    {
        ...
        return QString::fromUtf8(
            static_cast<const char*>(blob->getBufferPointer()), qsizetype(blob->getBufferSize()));
    }
    ...
    ParameterType reflectType(slang::TypeReflection* type)
    {
        ...
        if (kind == slang::TypeReflection::Kind::Vector) {
            ...
            if (scalar == slang::TypeReflection::ScalarType::Float32)
                return n == 2 ? ParameterType::Float2
                    : n == 3  ? ParameterType::Float3
                    : n == 4  ? ParameterType::Float4
                              : ParameterType::Unknown;
            if (scalar == slang::TypeReflection::ScalarType::Int32)
                return n == 2 ? ParameterType::Int2
                    : n == 3  ? ParameterType::Int3
                    : n == 4  ? ParameterType::Int4
                              : ParameterType::Unknown;
            if (scalar == slang::TypeReflection::ScalarType::UInt32)
                return n == 2 ? ParameterType::UInt2
                    : n == 3  ? ParameterType::UInt3
                    : n == 4  ? ParameterType::UInt4
                              : ParameterType::Unknown;
        }
        return ParameterType::Unknown;
    }
        ...
        const QString& group, QList<ParameterDescriptor>& out)
    {
        ...
        for (unsigned i = 0; i < unsigned(fieldCount); ++i) {
            ...
            if (name.isEmpty() || name.startsWith(QLatin1String("__")))
                continue; // compiler-internal
            const qsizetype offset
                = baseOffset + qsizetype(field->getOffset(SLANG_PARAMETER_CATEGORY_UNIFORM));
            ...
            if (kind == slang::TypeReflection::Kind::ConstantBuffer
                ...
                if (auto* elem = fieldTL->getElementTypeLayout())
                    collectParameters(elem, offset, name, out);
                continue;
            }
            ...
            d.step = (d.type == ParameterType::Float || d.type == ParameterType::Float2
                         || d.type == ParameterType::Float3 || d.type == ParameterType::Float4)
                ? QVariant(0.01)
                : QVariant(1);
            ...
            for (unsigned a = 0, n = var->getUserAttributeCount(); a < n; ++a) {
                ...
                auto argStr = [&](unsigned idx) -> QString {
                    ...
                    if (q.size() >= 2 && q.startsWith(QLatin1Char('"'))
                ...
                auto argNum = [&](unsigned idx, double& v) -> bool {
                    float f = 0.0f;
                    if (SLANG_SUCCEEDED(attr->getArgumentValueFloat(idx, &f))) {
                        v = f;
                        return true;
                    }
                    int iv = 0;
                    if (SLANG_SUCCEEDED(attr->getArgumentValueInt(idx, &iv))) {
                        v = iv;
                        return true;
                    }
                    return false;
            ...
            if (!units.isEmpty())
                d.label += QStringLiteral(" (") + units + QLatin1Char(')');
    ...
    QList<ParameterDescriptor> reflectParameters(slang::ProgramLayout* layout, qsizetype& byteSize)
    {
        ...
        if (tl->getKind() == slang::TypeReflection::Kind::ConstantBuffer
            ...
            if (auto* elem = tl->getElementTypeLayout())
                tl = elem;
        }
        ...
        std::stable_sort(
            out.begin(), out.end(), [](const ParameterDescriptor& a, const ParameterDescriptor& b) {
...
SlangCompiler::SlangCompiler(QObject* parent)
    : QObject(parent)
    , d(std::make_unique<SlangCompilerPrivate>())
{
}
SlangCompiler::~SlangCompiler() = default;
void SlangCompiler::setSearchPaths(QStringList paths)
{
    d->searchPaths = std::move(paths);
}
QStringList SlangCompiler::searchPaths() const
{
    return d->searchPaths;
}
void SlangCompiler::setSystemPrelude(const QString& source)
{
    d->systemPrelude = source;
}
QString SlangCompiler::systemPrelude() const
{
    ...
    const QString& vertexEntry, const QString& fragmentEntry)
{
    ...
    if (!d->global) {
        result.diagnostics = QStringLiteral("Failed to create Slang global session.");
        return result;
    }
    ...
    for (const auto& p : d->searchPaths)
        searchUtf8.push_back(p.toUtf8());
    for (const auto& p : searchUtf8)
        searchPtrs.push_back(p.constData());
    ...
    auto compileStage
        ...
        for (int target = 0; target < 5; ++target) {
            ...
            if (SLANG_FAILED(
                    linked->getEntryPointCode(epIndex, target, code.writeRef(), diag.writeRef()))) {
            ...
            if (target <= 2) {
                ...
                if (!codes[target].isEmpty())
                    ++produced;
            }
            if (kTargetName[target]) {
                const QString text = blobText(code);
                if (!text.isEmpty())
                    out.generated.insert(QString::fromLatin1(kTargetName[target]), text);
            }
        }
        if (produced == 0)
            return false;

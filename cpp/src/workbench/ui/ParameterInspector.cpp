...
namespace {
    ...
    QWidget* makeEditor(ShaderParameterModel* model, int row, QWidget* parent)
    {
        ...
        if (type == ParameterType::Bool) {
            ...
            QObject::connect(w, &QCheckBox::toggled, model,
                [model, idx](bool v) { model->setData(idx, v, ShaderParameterModel::ValueRole); });
            return w;
        }
        if (type == ParameterType::Int || type == ParameterType::UInt) {
            ...
            QObject::connect(w, &QSpinBox::valueChanged, model,
                [model, idx](int v) { model->setData(idx, v, ShaderParameterModel::ValueRole); });
            return w;
        }
        if (type == ParameterType::Float) {
            ...
            if (bounded
                && (widget == QLatin1String("slider") || widget == QLatin1String("angle"))) {
                auto* container = new QWidget(parent);
                auto* line = new QHBoxLayout(container);
                line->setContentsMargins(0, 0, 0, 0);
                auto* slider = new QSlider(Qt::Horizontal, container);
                slider->setRange(0, 1000);
                auto toTick = [dmin, dmax](double v) {
                    return int(qBound(0.0, (v - dmin) / (dmax - dmin) * 1000.0, 1000.0));
                };
                auto toVal
                    = [dmin, dmax](int t) { return dmin + (double(t) / 1000.0) * (dmax - dmin); };
                slider->setValue(toTick(value.toDouble()));
                spin->setParent(container);
                line->addWidget(slider, 1);
                line->addWidget(spin);
                QObject::connect(spin, &QDoubleSpinBox::valueChanged, model,
                    [model, idx, slider, toTick](double v) {
                        {
                            QSignalBlocker b(slider);
                            slider->setValue(toTick(v));
                        }
                        model->setData(idx, v, ShaderParameterModel::ValueRole);
                    });
                QObject::connect(slider, &QSlider::valueChanged, spin,
                    [spin, toVal](int t) { spin->setValue(toVal(t)); });
                return container;
            }

            spin->setParent(parent);
            QObject::connect(spin, &QDoubleSpinBox::valueChanged, model, [model, idx](double v) {
                model->setData(idx, v, ShaderParameterModel::ValueRole);
            });
            return spin;
        }
        ...
        int count = type == ParameterType::Float2 ? 2
            : type == ParameterType::Float3       ? 3
            : type == ParameterType::Float4       ? 4
                                                  : 0;
        if (count) {
            auto* container = new QWidget(parent);
            auto* line = new QHBoxLayout(container);
            ...
            if (type == ParameterType::Float2) {
                auto v = value.value<QVector2D>();
                vals = { v.x(), v.y() };
            }
            if (type == ParameterType::Float3) {
                auto v = value.value<QVector3D>();
                vals = { v.x(), v.y(), v.z() };
            }
            if (type == ParameterType::Float4) {
                auto v = value.value<QVector4D>();
            ...
            for (int c = 0; c < count; ++c) {
                ...
                spin->setDecimals(5);
                spin->setValue(vals[c]);
                line->addWidget(spin);
                QObject::connect(spin, &QDoubleSpinBox::valueChanged, model,
                    [model, idx, shared, c, type](double x) {
                        (*shared)[c] = x;
                        QVariant v;
                        if (type == ParameterType::Float2)
                            v = QVariant::fromValue(
                                QVector2D(float((*shared)[0]), float((*shared)[1])));
                        if (type == ParameterType::Float3)
                            v = QVariant::fromValue(QVector3D(
                                float((*shared)[0]), float((*shared)[1]), float((*shared)[2])));
                        if (type == ParameterType::Float4)
                            v = QVariant::fromValue(QVector4D(float((*shared)[0]),
                                float((*shared)[1]), float((*shared)[2]), float((*shared)[3])));
                        model->setData(idx, v, ShaderParameterModel::ValueRole);
                    });
            }
            return container;
...
void ParameterInspector::setModel(ShaderParameterModel* model)
{
    ...
    if (m_model) {
        ...
        connect(m_model, &ShaderParameterModel::layoutChangedByShader, this,
            &ParameterInspector::rebuild);
    }
    rebuild();
    emit modelChanged();
}

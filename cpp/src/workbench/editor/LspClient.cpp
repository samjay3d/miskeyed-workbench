...
LspClient::~LspClient()
{
    if (m_proc && m_proc->state() != QProcess::NotRunning) {
        ...
        writeMessage({ { "jsonrpc", QStringLiteral("2.0") }, { "method", QStringLiteral("exit") },
            { "params", QJsonValue::Null } });
...
void LspClient::start(const QString& serverPath)
{
    ...
    QJsonObject textDocument { { QStringLiteral("completion"), completion },
    ...
    QJsonObject params { { QStringLiteral("processId"), QJsonValue::Null },
        { QStringLiteral("rootUri"), QJsonValue::Null },
        { QStringLiteral("clientInfo"),
            QJsonObject { { QStringLiteral("name"), QStringLiteral("slang-qt") } } },
    ...
    postNotification(QStringLiteral("textDocument/didOpen"),
        { { QStringLiteral("textDocument"),
            QJsonObject { { QStringLiteral("uri"), uri },
                ...
                { QStringLiteral("version"), 1 }, { QStringLiteral("text"), text } } } });
}
...
{
    return QJsonObject { { QStringLiteral("textDocument"),
                             QJsonObject { { QStringLiteral("uri"), uri } } },
...
{
    postRequest(QStringLiteral("textDocument/completion"), positionParams(uri, line, character),
        std::move(cb));
}
...
{
    postRequest(
        QStringLiteral("textDocument/hover"), positionParams(uri, line, character), std::move(cb));
}

void LspClient::requestSignatureHelp(const QString& uri, int line, int character, JsonCallback cb)
{
    postRequest(QStringLiteral("textDocument/signatureHelp"), positionParams(uri, line, character),
        std::move(cb));
}

void LspClient::requestDefinition(const QString& uri, int line, int character, JsonCallback cb)
{
    postRequest(QStringLiteral("textDocument/definition"), positionParams(uri, line, character),
        std::move(cb));
}
...
{
    if (!m_ready) {
        m_queued.append([this, method, params] { postNotification(method, params); });
        return;
    }
    writeMessage({ { QStringLiteral("jsonrpc"), QStringLiteral("2.0") },
...
{
    if (!m_ready) {
        m_queued.append([this, method, params, cb] { postRequest(method, params, cb); });
        return;
    }
...
void LspClient::sendRequestNow(const QString& method, const QJsonObject& params, JsonCallback cb)
{
    ...
    writeMessage(
        { { QStringLiteral("jsonrpc"), QStringLiteral("2.0") }, { QStringLiteral("id"), id },
            { QStringLiteral("method"), method }, { QStringLiteral("params"), params } });
}
...
void LspClient::handleMessage(const QJsonObject& message)
{
    ...
    if (hasId && hasMethod) {
        ...
        if (method == QStringLiteral("workspace/configuration")) {
            const QJsonArray items = message.value(QStringLiteral("params"))
                                         .toObject()
    ...
    if (hasMethod) {
        ...
        if (method == QStringLiteral("textDocument/publishDiagnostics")) {
            ...
            for (const QJsonValue& v : params.value(QStringLiteral("diagnostics")).toArray()) {
                ...
                LspDiagnostic diag;
                diag.range = { s.value(QStringLiteral("line")).toInt(),

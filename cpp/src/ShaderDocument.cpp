...
ShaderDocument::ShaderDocument(QObject* parent)
    : QObject(parent), m_compiler(this), m_parameters(this), m_graph(this)
{
    ...
    connect(&m_parameters, &ShaderParameterModel::packedRangeChanged, this, [this](int, int){
...
void ShaderDocument::initializeGraph()
{
    ...
    m_layoutNode = m_graph.ensureNode(QStringLiteral("shader:parameter-layout"), NodeKind::ParameterLayout);
    m_valuesNode = m_graph.ensureNode(QStringLiteral("shader:parameter-values"), NodeKind::ParameterValues);
...
{
    if (m_fileUrl == url) return;
    m_fileUrl = url; emit fileUrlChanged();
}
...
void ShaderDocument::setSource(const QString& source)
{
    ...
    emit sourceChanged();
    if (m_live) m_compileTimer.start();
}

void ShaderDocument::setLive(bool live)
{
    if (m_live == live) return;
    m_live = live; emit liveChanged();
    if (live && !m_source.isEmpty()) m_compileTimer.start();
}

bool ShaderDocument::load()
{
    if (!m_fileUrl.isLocalFile()) return false;
    QFile f(m_fileUrl.toLocalFile());
    if (!f.open(QIODevice::ReadOnly)) { setDiagnostics(f.errorString()); return false; }
...
bool ShaderDocument::save()
{
    ...
    if (!f.open(QIODevice::WriteOnly)) { setDiagnostics(f.errorString()); return false; }
    f.write(m_source.toUtf8());
    if (!f.commit()) { setDiagnostics(f.errorString()); return false; }
    return true;
}

void ShaderDocument::compile() { m_compileTimer.stop(); compileNow(); }

void ShaderDocument::compileNow()
{
    if (m_compiling) return;
    m_compiling = true; emit compilingChanged();
    const QString virtualPath = m_fileUrl.isLocalFile() ? m_fileUrl.toLocalFile() : QStringLiteral("user_shader.slang");
    QElapsedTimer timer; timer.start();
    const CompileResult r = m_compiler.compileFullscreen(m_source, virtualPath);
    m_lastCompileMs = int(timer.elapsed());
    m_compiling = false; emit compilingChanged();
    setDiagnostics(r.diagnostics);
    if (!r.ok) { emit compileFailed(r.diagnostics); return; }
    ...
    if (oldLayout != r.reflectionDigest) {
        ...
        m_graph.setPayload(m_uiNode, r.reflectionDigest); // UI is reflection-derived; metadata will extend this digest.
        m_graph.setPayload(m_valuesNode, m_parameters.packedBytes());
    }
    ...
    for (const QString& name : {QStringLiteral("HLSL"), QStringLiteral("GLSL"),
        ...
        if (vs.isEmpty() && fs.isEmpty()) continue;
        QString combined;
...
{
    if (m_diagnostics == text) return;
    m_diagnostics = std::move(text); emit diagnosticsChanged();
}

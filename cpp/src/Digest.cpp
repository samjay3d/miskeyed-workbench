...
inline quint64 rotr(quint64 x, int n) { return (x >> n) | (x << (64 - n)); }
...
struct Blake2b {
    ...
    void compress(const quint8* block) {
        quint64 m[16], v[16];
        for (int i = 0; i < 16; ++i) m[i] = qFromLittleEndian<quint64>(block + i * 8);
        for (int i = 0; i < 8; ++i) { v[i] = h[i]; v[i + 8] = IV[i]; }
        v[12] ^= t0; v[13] ^= t1; v[14] ^= f0;
    ...
    void update(QByteArrayView input) {
        ...
        while (n) {
            ...
            if (used == buffer.size() && n > 0) {
                increment(128); compress(buffer.data()); used = 0;
            }
        }
    }
    ...
    QByteArray finish() {
        ...
        quint8 tmp[64];
        for (int i = 0; i < 8; ++i) qToLittleEndian<quint64>(h[i], tmp + i * 8);
...
Digest::Digest() : m_bytes(Size, '\0') {}
Digest::Digest(QByteArray bytes) : m_bytes(std::move(bytes))
{
    if (m_bytes.size() != Size) throw std::invalid_argument("Digest must be exactly 32 bytes");
}

Digest Digest::hash(QByteArrayView bytes)
{
    Blake2b b; b.update(bytes); return Digest(b.finish());
}
...
{
    for (char c : m_bytes) if (c != 0) return false;
    return true;
}

...
bool DependencyGraph::contains(const QString& key) const { return m_byKey.contains(key); }
NodeId DependencyGraph::nodeId(const QString& key) const { return m_byKey.value(key, 0); }

QString DependencyGraph::digestHex(NodeId id) const
{
    if (const auto* n = node(id)) return n->merkleDigest.hex();
    return {};
}

quint32 DependencyGraph::dirtyFlags(NodeId id) const
{
    if (const auto* n = node(id)) return quint32(n->dirty.toInt());
    return 0;
}
...
QStringList DependencyGraph::dependencies(NodeId id) const
{
    ...
    if (const auto* n = node(id)) {
        for (NodeId d : n->dependencies) if (const auto* dep = node(d)) out << dep->key;
    }
    return out;
}
...
{
    if (const auto* n = node(id)) return n->payload;
    return {};
}
...
{
    switch (kind) {
    case NodeKind::UiSchema: return UiDirty;
    case NodeKind::ParameterValues: return UniformDirty;
    case NodeKind::Resource: return ResourceDirty;
    case NodeKind::BindingLayout: return BindingDirty | PipelineDirty;
    case NodeKind::Source:
    case NodeKind::Module:
    case NodeKind::EntryPoint: return ShaderDirty | PipelineDirty;
    case NodeKind::ParameterLayout: return UiDirty | UniformDirty | BindingDirty | PipelineDirty;
    case NodeKind::RenderState:
    case NodeKind::Pipeline: return PipelineDirty;
    }
...
{
    auto* n = node(id);
    if (!n || n->payload == payload) return false;
...
bool DependencyGraph::setDependencies(NodeId id, const QList<NodeId>& deps)
{
    ...
    std::sort(canonical.begin(), canonical.end(), [&](NodeId a, NodeId b) {
        const auto* na = node(a); const auto* nb = node(b);
        return na && nb ? na->key < nb->key : a < b;
    });
    canonical.erase(std::unique(canonical.begin(), canonical.end()), canonical.end());
    if (canonical == n->dependencies) return false;

    for (NodeId old : n->dependencies) if (auto* dep = node(old)) dep->dependents.remove(id);
    n->dependencies = canonical;
    for (NodeId depId : canonical) {
        if (depId == id) throw std::logic_error("DependencyGraph self-cycle");
        if (auto* dep = node(depId)) dep->dependents.insert(id);
    }
    recomputeAndPropagate(id);
    return true;
}
...
Digest DependencyGraph::recompute(NodeId id, QSet<NodeId>& active)
{
    auto* n = node(id);
    if (!n) return {};
    if (active.contains(id)) throw std::logic_error("DependencyGraph cycle detected");
    active.insert(id);
...
void DependencyGraph::recomputeAndPropagate(NodeId start)
{
    ...
    QSet<NodeId> queued; for (NodeId q : queue) queued.insert(q);
    while (!queue.isEmpty()) {
        ...
        if (before != cur->merkleDigest) {
            for (NodeId p : cur->dependents) if (!queued.contains(p)) { queue.push_back(p); queued.insert(p); }
        }
    }
    emit graphChanged();
}
...
{
    if (auto* n = node(id)) n->dirty &= ~flags;
}

void DependencyGraph::markAllClean()
{
    for (auto it = m_nodes.begin(); it != m_nodes.end(); ++it) it->dirty = {};
}

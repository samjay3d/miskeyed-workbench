...
{
    return parent.isValid() ? 0 : m_descriptors.size();
}

QHash<int, QByteArray> ShaderParameterModel::roleNames() const
{
    return { { NameRole, "name" }, { LabelRole, "label" }, { GroupRole, "group" },
...
QVariant ShaderParameterModel::data(const QModelIndex& index, int role) const
{
    ...
    case Qt::DisplayRole:
    case LabelRole:
        return d.label.isEmpty() ? d.name : d.label;
    case NameRole:
        return d.name;
    case GroupRole:
        return d.group;
...
void ShaderParameterModel::setDescriptors(
    QList<ParameterDescriptor> descriptors, qsizetype byteSize)
{
    QHash<QString, QVariant> preserved;
    QHash<QString, ParameterType> oldTypes;
    for (int i = 0; i < m_descriptors.size(); ++i) {
        preserved.insert(m_descriptors[i].name, m_values[i]);
        oldTypes.insert(m_descriptors[i].name, m_descriptors[i].type);
    }
    ...
    for (int i = 0; i < m_descriptors.size(); ++i) {
        ...
        if (preserved.contains(d.name) && oldTypes.value(d.name) == d.type)
            v = preserved.value(d.name);
...
bool ShaderParameterModel::setValue(const QString& name, const QVariant& value)
{
    for (int i = 0; i < m_descriptors.size(); ++i)
        if (m_descriptors[i].name == name)
            return writeValue(i, value, true);
    return false;
}
...
{
    for (int i = 0; i < m_descriptors.size(); ++i)
        writeValue(i, m_descriptors[i].defaultValue, false);
    if (!m_bytes.isEmpty())
        emit packedRangeChanged(0, int(m_bytes.size()));
    if (!m_descriptors.isEmpty())
        emit dataChanged(index(0), index(m_descriptors.size() - 1), { ValueRole });
}
...
bool ShaderParameterModel::writeValue(int row, const QVariant& value, bool emitSignals)
{
    ...
    if (normalized == m_values[row])
        return false;
    m_values[row] = normalized;
    if (!packValue(d, normalized, m_bytes))
        return false;
    if (emitSignals) {
...
QVariant ShaderParameterModel::normalizedValue(const ParameterDescriptor& d, const QVariant& value)
{
    if (d.type == ParameterType::Bool)
        return value.toBool();
    if (d.type == ParameterType::Float || d.type == ParameterType::Int
        || d.type == ParameterType::UInt)
        return value;
    return value;
}

bool ShaderParameterModel::packValue(
    const ParameterDescriptor& d, const QVariant& v, QByteArray& bytes)
{
    if (d.offset < 0 || d.offset + d.size > bytes.size())
        return false;
    char* dst = bytes.data() + d.offset;
    auto f = [&](float x, int slot) {
        if (qsizetype((slot + 1) * 4) <= d.size)
            std::memcpy(dst + slot * 4, &x, 4);
    };
    auto i = [&](qint32 x, int slot) {
        if (qsizetype((slot + 1) * 4) <= d.size)
            std::memcpy(dst + slot * 4, &x, 4);
    };
    auto u = [&](quint32 x, int slot) {
        if (qsizetype((slot + 1) * 4) <= d.size)
            std::memcpy(dst + slot * 4, &x, 4);
    };
    ...
    case ParameterType::Float2: {
        auto x = v.value<QVector2D>();
        f(x.x(), 0);
        f(x.y(), 1);
        return true;
    }
    case ParameterType::Float3: {
        auto x = v.value<QVector3D>();
        f(x.x(), 0);
        f(x.y(), 1);
        f(x.z(), 2);
        return true;

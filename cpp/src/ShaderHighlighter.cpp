...
ShaderHighlighter::ShaderHighlighter(QTextDocument* parent) : QSyntaxHighlighter(parent)
{
    ...
    auto add = [this](const QString& p, const QColor& c, bool italic = false, bool bold = false) {
        ...
        if (italic) r.format.setFontItalic(true);
        if (bold) r.format.setFontWeight(QFont::DemiBold);
    ...
    static const char* const types[] = {
        ...
        "float2","float3","float4","float2x2","float3x3","float4x4","float3x4","float4x3",
        "int2","int3","int4","uint2","uint3","uint4","half2","half3","half4",
        "double2","double3","double4","bool2","bool3","bool4","matrix","vector",
        "vec2","vec3","vec4","ivec2","ivec3","ivec4","mat2","mat3","mat4","sampler2D",
        ...
        "StructuredBuffer","RWStructuredBuffer","ConstantBuffer","ByteAddressBuffer","Buffer",
    };
    QString typeAlt;
    for (const char* t : types) { if (!typeAlt.isEmpty()) typeAlt += QLatin1Char('|'); typeAlt += QLatin1String(t); }
    add(QStringLiteral("\\b(?:%1)\\b").arg(typeAlt), kType, false, true);

    // Keywords.
    static const char* const keywords[] = {
        "if","else","for","while","do","switch","case","default","break","continue","return",
        "discard","struct","cbuffer","tbuffer","typedef","namespace","import","module","using",
        "in","out","inout","ref","const","static","uniform","varying","groupshared","precise",
        "export","public","private","internal","extension","interface","associatedtype","this",
        "true","false","nullptr","let","var","func","property","enum","typealias","where","is",
        "as","new","sizeof","register","packoffset","layout","flat","noperspective","centroid",
        "sample","column_major","row_major","volatile","globallycoherent","snorm","unorm",
    };
    QString kwAlt;
    for (const char* k : keywords) { if (!kwAlt.isEmpty()) kwAlt += QLatin1Char('|'); kwAlt += QLatin1String(k); }
